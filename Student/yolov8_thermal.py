...
class YOLOv8Thermal(nn.Module):
    ...
    def forward(
        ...
        # Detection heads
        # Default behavior:
        # - training: List[Tensor] (Ultralytics v8DetectionLoss format)
        # - eval: Dict[str, Dict[str, Tensor]] (split predictions)
        predictions = self.head(fused_features)

        # If caller needs features, always return a dict, even in training.
        # This keeps default Ultralytics-loss compatibility when return_features=False,
        # while enabling distillation to access fused features during training.
        
        if return_features:
        # if True:
            if self.training:
            # if True:
                # In training mode, `predictions` is already the Ultralytics-format list.
                predictions_ultralytics = predictions
                # Also build split predictions (cls/reg/obj) for optional debugging.
                # We temporarily run the heads in "inference style" without changing module mode.
                split = {}
                for scale in self.head.scale_names:
                    split[scale] = self.head.heads[scale](fused_features[scale])
                output = {
                    'predictions': split,
                    'predictions_ultralytics': predictions_ultralytics,
                    'backbone_features': features,
                    'fused_features': fused_features,
                }
                return output

            # Eval/inference mode: rebuild ultralytics format from split predictions
            predictions_ultralytics = []
            for scale in self.head.scale_names:
                scale_pred = predictions[scale]
                # Concatenate reg and cls like in training
                combined = torch.cat([scale_pred['reg'], scale_pred['cls']], dim=1)
                predictions_ultralytics.append(combined)
        ...
        # No features requested: preserve original return shapes
        if self.training:
...
if __name__ == "__main__":
    ...
    print("items of dict : ",output.keys())
    print("output of detection head",len(output["predictions_ultralytics"]))
    print("output shape of detection head 0",output["predictions_ultralytics"][0].shape)
    predictions = output["predictions_ultralytics"]
    print(f"  Shapes: {[item.shape for item in predictions]}")


    # sampled_output = model._to_ultralytics_format(output['predictions'])
    sampled_output = output["predictions"]
    print("datatype of sampled_output : ",type(sampled_output))
    for keys, values in sampled_output.items():
        print(f"{keys} : {type(values)}")
        for key, value in values.items():
            print(f"{key}:{value.shape}")


    # print(f"\n printed output shape: {type(sampled_output)}, {len(sampled_output)} scales")
    # print(f"  Shapes: {[item.shape for item in sampled_output]}")
